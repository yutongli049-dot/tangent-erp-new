...
import { 
  ...
  Loader2, ArrowUpRight, ChevronRight, MoreHorizontal,
  X, Clock, MapPin, BookOpen, User
} from "lucide-react";
...
export default function Home() {
  ...
  const handleDayClick = (day: Date, bookings: any[]) => {
    if (bookings.length === 0) return; // 没课就不弹窗
    setSelectedDate(day);
    ...
    <main className="flex min-h-screen flex-col bg-slate-50/50 font-sans text-slate-900 pb-10">
      ...
      <div className="mx-auto w-full max-w-6xl px-6 py-8">
        <div className="grid grid-cols-12 gap-6">
          ...
          {/* Row 2: Cash Flow (Clickable) + Quick Actions */}
          {/* ✅ 现金流卡片变为可点击链接 */}
                ...
                {daysInMonth.map((day) => {
                  ...
                  return (
                    <div 
                      ...
                      className={`
                        min-h-[100px] rounded-xl border p-2 flex flex-col gap-1 transition-all
                        ${isTodayDate ? 'bg-indigo-50/50 border-indigo-200' : 'bg-white border-slate-100'}
                        ${hasBooking ? 'hover:border-indigo-300 hover:shadow-sm cursor-pointer hover:scale-[1.02]' : ''}
                      `}
                    >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
          <div className="w-full max-w-md bg-white rounded-2xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
                ...
                <div key={b.id} className="p-4 rounded-xl border border-slate-100 bg-white shadow-sm flex flex-col gap-2">
                  ...
                  <div className="grid grid-cols-2 gap-2 mt-2 pt-2 border-t border-slate-50">
                    <div className="flex items-center gap-2 text-xs text-slate-600">
                      ...
                      {new Date(b.start_time).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})} ({b.duration}h)
                    </div>
            ...
            <div className="p-4 bg-slate-50 border-t border-slate-100">
              <Link href="/bookings">
                <Button className="w-full bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl">前往排课表管理</Button>
              </Link>
            </div>
          </div>
        </div>
